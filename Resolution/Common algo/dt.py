...
def dt_algo(x_train, y_train, x_test, y_test, test_df, process):
    dt = DtClassifier(x_train, y_train, process)
    clf = dt.test_model(x_test, y_test)
    if process == "file":
        return dt.predict_data_file(clf, test_df)
    else:
        return dt.predict_data_row(clf, test_df)
...
class DtClassifier:
    ...
    def test_model(self, x_test, y_test):
        ...
        print("Accuracy of decision tree model:  %.3f" % accuracy_score(y_test, y_pred))
        ...
        plt.savefig(f"./images/confusion_matrix_dt_{self.process}.png", dpi=75, format="png")
        plt.show()
        return clf
